...
def train(network, train_loader, optimizer, schedule, epochs, n=1, device='cuda'):
    ...
    for i in tqdm.tqdm(range(epochs)):
        ...
        for imgs, labels in train_loader:
            ...
            if batch % 10 == 0:
                print("Epoch:", i + 1, "| Batch:", batch, " | Loss:" , loss_train / 10 )
                loss_train = 0.0
...
adam = torch.optim.Adam(network.parameters(), lr=0.01, weight_decay=1e-04)
SGD = torch.optim.SGD(network.parameters(), lr=0.1, weight_decay=1e-04, momentum=0.9)

schedule = lr_scheduler.ExponentialLR(adam, gamma=args.gamma)
schedule2 = lr_scheduler.ReduceLROnPlateau(adam, factor = 0.1, patience=5)
